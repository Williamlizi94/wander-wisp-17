...
serve(async (req) => {
  ...
  try {
    ...
    const prompt = `你是一个专业的全球旅行规划师，精通世界各地的旅游目的地。请为以下旅行生成详细攻略：
...
要求：
- 推荐3个住宿区域（不要写具体酒店名，不要写价格），说明该区域的优势如交通便利、环境优美、靠近景点等，并提供2-3个标签
- 每天安排上午、下午、晚上3个时段
- 每天推荐5-8个当地特色美食，只写菜品名不写具体店名，推荐地点选在当天行程景点附近的区域
- 天气数据请根据${city}该季节的典型天气给出合理估计
- 交通建议要具体实用，考虑当地实际交通情况
- 如果用户偏好包含"自驾"，或目的地在美国/加拿大/澳大利亚等自驾文化盛行的地区，交通建议应以自驾为主，包含驾车时间、推荐路线、停车建议等
- 美国城市之间通常需要自驾，请规划合理的自驾路线和每日驾车时间，避免单日驾车超过4小时
- Plan B要有具体的室内替代方案
- 根据出行人群特点调整景点和活动推荐
- 返回${dayCount}天的完整攻略
